...
### [v4.75 / v1.9.2] 终放打包与版本校对
- **UI 同步**：修正了 `popup.html` 中硬编码的版本号，确保在 `popup.js` 加载前也能显示正确的基础版本。
- **打包准备**：清理冗余文件，准备生成最新的分发 zip 包。

### [v4.74 / v1.9.1] 搜索全选系统补完
- **全量兼容**：将 `selectAllVisible` 的选择器范围扩展至 `cdk-tree-node` 和 `mat-tree-node`，支持层级结构下的全选。
- **鲁棒点击**：优化点击机制，优先触发隐藏的 `input` 以绕过可能的 UI 遮挡，并增加 `offsetParent` 检查以确保元素真实可见。
- **逻辑优化**：由“同步勾选”改为“增量勾选”，即仅选中可见且未选的项目，不再自动取消隐藏项目的勾选，符合用户叠加搜索选择的直觉。
